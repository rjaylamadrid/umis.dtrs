...
class Payroll{
    ...
    protected static function set_page($no, $sheets) {
        ...
        $irow = self::get_total($row+12, $irow+1, $no==$sheets, $no);
    ...
    protected static function get_total($start, $row, $last = false, $no) {
        if ($last && $no=='1') {
            ...
            self::set_total(self::$columns, $start, $row);
        } else{
            $irow = $last ? $row+1 : $row;
            ...
            if($last) {
                ...
                self::set_total(self::$columns, $start, $row+1, self::$rows);
            }
        }
        return $irow;
    }

    protected static function set_total ($columns, $start, $row, $rows = null) {
        ...
        Excel::set_border('A'.$row.':'.self::$lastColumn.$row, 'inside', 'thin');
        Excel::set_border('A'.$row.':'.self::$lastColumn.$row, 'outline', 'medium');
    }
