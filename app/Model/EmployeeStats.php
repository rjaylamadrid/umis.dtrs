...
class EmployeeStats {
    ...
    public static function employees_count ($status = 1) {
        $result = DB::fetch_row ("SELECT COUNT(*) as total FROM (SELECT active_status FROM tbl_employee a, tbl_employee_status b WHERE a.no = b.employee_id AND b.campus_id = ? AND b.no = (SELECT no FROM tbl_employee_status WHERE employee_id = a.no ORDER BY date_start DESC LIMIT 0,1)) a WHERE active_status = ?", [self::$campus, $status]);
        return $result['total'];
    }
