...
class LoginController extends Controller {
    ...
    protected function find ($id) {
        $user = DB::fetch_row ("SELECT * FROM tbl_user_employee a, tbl_employee_status b WHERE a.employee_username = ? and a.employee_id = b.employee_id AND b.active_status = '1' ORDER BY b.date DESC", $id)[0];
        if ($user) {
            $this->tmp_user = $user;
            return $user;
        }
